...
var OVERLAP_SQL_PREFIX = 'SELECT _id, uuid FROM ' + BUCKET.name +
    ' WHERE (subnet >> $1 OR subnet_start << $2)';

var OVERLAP_NORMAL_NET_SQL = OVERLAP_SQL_PREFIX +
    ' AND fabric != true;';

var OVERLAP_FABRIC_NET_SQL = OVERLAP_SQL_PREFIX +
    ' AND fabric = true AND vlan_id = $3 AND vnet_id = $4;';
...
function validateProvisionRangeUpdate(opts, params, parsed, cb) {
    ...
    var toValidate = {
        vlan_id: params.network.params.vlan_id,
...
function listNetworks(opts, callback) {
    ...
    validateListNetworks(opts.params, function (err, params) {
        ...
        if (ownerUUID) {
            // Match only networks with that owner_uuid, which is more specific
            // than a provisionable_by query, and overrides it if specified.
        ...
        if (params.name) {
            ...
            names.forEach(function (name) {
                ...
                if (ownerUUID) {
                    ...
                    nameStrs.push(ownerUUID + ':' + name);
                } else if (provisionableBy) {
                    if (!params.fabric) {
                        nameStrs.push(globalName);
                    }
                    nameStrs.push('*:' + name);
                    ...
                    if (params.fabric) {
                        ...
                        // Only search for global networks: searches w/o a user
                        // UUID specified are coming from an operator, and we
                        // don't want users' fabrics showing up in their
                        // searches for things like name=admin.
                        nameStrs.push(globalName);
                    }
                }
            });
...
function findContainingNetworks(opts, vlan_id, nic_tag, ip, callback) {
    var req = opts.app.moray.sql(CONTAINING_NET_SQL,
        [ip.toString(), vlan_id, nic_tag]);
    var uuids = [];
    req.on('record', function (r) {
        uuids.push(r.uuid);
    });
